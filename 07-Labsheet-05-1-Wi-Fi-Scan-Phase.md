...
| :---: | :--- | :---: | :---: | :---: |
| **5.1.1** | สแกนทั่วไปทุก Channel | ESP_OK (Success) | 5 | ~2,500 ms |
| **5.1.2** | กำหนดสแกนเฉพาะ Channel 1 | ESP_OK (Success) | 2 | ~300 - 500 ms |
| **5.1.3** | กำหนดสแกน SSID ที่มีจริง | ESP_OK (Success) | 1 | ~1,500 ms |
| **5.1.4** | กำหนดสแกน SSID ที่ไม่มีจริง | ESP_OK (Success) | 0 | 2498 ms |
...
| :---: | :--- | :--- | :---: | :---: | :--- |
| 1 | KMITL-WIFI | 78:17:BE:C0:7D:A1 | -57  dBm | 1 | OPEN (No Password) |
| 2 | KMITL-WIFI | 78:17:BE:C0:66:21 | -81  dBm | 1 | OPEN (No Password) |
| 3 | KMITL-WIFI | 78:17:BE:C0:66:61 | -82  dBm | 11 | OPEN (No Password) |
| 4 | KMITL-WIFI | 78:17:BE:A9:94:E1 | -83  dBm | 6 | OPEN (No Password) |
| 5 | KMITL-WIFI | 78:17:BE:C0:72:61 | -86  dBm | 1 | OPEN (No Password) |
...
`การช่วยลดเวลา: ปกติการสแกนแบบครอบคลุมทั้งหมด (ข้อ 5.1.1 กำหนด .channel = 0) บอร์ด ESP32 จะต้องสลับคลื่นวิทยุและรอฟังสัญญาณทีละช่องตั้งแต่ช่อง 1 ถึง 13 ทำให้ใช้เวลาสะสมนาน แต่การระบุช่องความถี่เดี่ยว (เช่น .channel = 1) จะทำให้ฮาร์ดแวร์โฟกัสรับสัญญาณเฉพาะช่องนั้นทันทีโดยไม่ต้องวนข้ามช่อง ส่งผลให้ระยะเวลาในการสแกนลดลงอย่างรวดเร็ว
ข้อจำกัด: หากระบุช่องความถี่ผิด หรือเครือข่ายเป้าหมายไม่ได้ทำงานอยู่ในช่องนั้น บอร์ดจะไม่สามารถค้นหาเจอเลยเนื่องจากถูกจำกัดให้มองหาแค่ช่องเดียว`
...
`ฟังก์ชัน esp_wifi_scan_start() ส่งคืนค่าเป็น ESP_OK (0x0) เนื่องจากคำสั่งทำงานสำเร็จตามกระบวนการของไดรเวอร์
ฟังก์ชัน esp_wifi_scan_get_ap_num(&ap_count) ส่งคืนค่าเป็น ESP_OK พร้อมกับค่าตัวแปร ap_count = 0 (ไม่พบเครือข่าย)
ฟังก์ชัน esp_wifi_scan_get_ap_records() จะไม่ถูกเรียกใช้งานในกรณีนี้ เนื่องจากโปรแกรมมีการเช็คเงื่อนไข if (ap_count == 0) เพื่อข้ามขั้นตอนการดึงข้อมูลและจองหน่วยความจำ ป้องกันข้อผิดพลาดที่อาจเกิดขึ้น`
...
`ค่า -45 dBm มีความแรงและความเสถียรมากกว่า เนื่องจากค่า RSSI (Received Signal Strength Indicator) ในหน่วยเดซิเบลเมตร ($dBm$) ยิ่งตัวเลขติดลบน้อยลง (เข้าใกล้ 0 มากขึ้น) แสดงว่าสัญญาณยิ่งมีความเข้มข้นสูง`
...
`การรู้รูปแบบการเข้ารหัสความปลอดภัย (เช่น WIFI_AUTH_OPEN, WIFI_AUTH_WPA2_PSK, หรือ WIFI_AUTH_WPA3_PSK) เป็นข้อมูลจำเป็นในการกำหนดค่าพารามิเตอร์ของรหัสผ่านและการจับมือความปลอดภัย (Handshake) ให้ถูกต้องตรงกับ Access Point เป้าหมายใน Authentication & Association Phase หากระบุโหมดความปลอดภัยไม่สอดคล้องกัน การเชื่อมต่อจะถูกปฏิเสธทันที`
